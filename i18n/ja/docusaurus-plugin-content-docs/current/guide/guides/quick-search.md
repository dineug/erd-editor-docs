...
`Database`、`Import`、`Auto Layout` などの項目はサブメニューを開きます。サブメニューの項目が一覧を置き換え、検索ボックスはクリアされます。絞り込みは常に表示中の一覧だけに適用され、上の階層へ戻る方法はありません。最初からやり直すにはパレットを閉じて開き直します。
...
ERD タブでは、データベースベンダー、`Import` と `Export`、`New Table` と `New Memo`、4 種類のリレーションシップ、`Auto Layout`、そして名前順に並んだテーブルごとの項目を表示します。名前が空のテーブルは `unnamed` として表示され、実行するとそのテーブルまでスクロールして選択します。  
