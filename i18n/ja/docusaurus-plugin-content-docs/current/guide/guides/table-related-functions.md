...
前半はテーブルとメモの操作、続いてテーブルのプロパティ、インデックス、テーブルの自動配置、データベースベンダー、最後にキャンバス自体のキャンバスツールバー、拡大・縮小、移動、Diff Viewer を扱います。
...
配置が落ち着くまではダイアグラム全体のプレビューが開き、表示される通知に `Apply` と `Cancel` があります。  
