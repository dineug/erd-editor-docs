...
`tableShadow` と `memoShadow` は、ERD キャンバス上でテーブルとメモが落とす影の色です。ダークプリセットではどちらも `transparent` で、`setTheme` に `none` や空文字列を渡したときと同じく影は描画されません。ライトプリセットでは `rgba(0, 0, 0, 0.18)` を使います。
