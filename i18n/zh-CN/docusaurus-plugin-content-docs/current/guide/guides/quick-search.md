...
`Database` 或 `Import` 之类的条目会打开子菜单，其条目会替换当前列表，搜索框也会被清空。过滤始终只作用于当前显示的列表，并且无法返回上一级，因此需要关闭并重新打开命令面板才能重新开始。
...
在 ERD 标签页中，列表包含数据库厂商、`Import` 与 `Export`、`New Table` 与 `New Memo`、四种关系类型、`Automatic Table Placement`，以及每个表各一个的条目，按名称排序。名称为空的表会显示为 `unnamed`，执行该条目会滚动到该表并将其选中。  
