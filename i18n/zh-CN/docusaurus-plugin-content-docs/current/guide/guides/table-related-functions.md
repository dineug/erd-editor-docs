...
先是表与备注的相关命令，然后是表属性、索引、自动布局和数据库厂商，最后是画布本身：画布工具栏、缩放、平移和 Diff Viewer。
...
其余三种在主线程之外一次算完，没有过程可看：计算期间会显示一条提示，算好的布局会落在图形已经占据的区域中央。`Cancel` 或 `Escape` 会丢弃你不再等待的布局；不运行 shared worker 的宿主环境则会以 `Could not place tables` 收场，参见 [Web Worker](../../api/installation.md#web-worker)。
...
`Ctrl + O` (Windows/Linux) 或 `⌘ + O` (Mac) 会保持屏幕中心不变，直接回到 `100%`。
...
平移的范围由图决定：你可以把表推到屏幕之外一点，但无法继续平移到更远的空白处。
...
如果平移之后屏幕上没有留下任何表或备注，底部边缘会出现一个胶囊形按钮，用箭头指出最近的对象在哪个方向、有多远。点击它即可把该对象移到屏幕中央。
