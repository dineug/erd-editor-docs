---
sidebar_position: 2
description: JSON 또는 스키마 SQL 가져오기와, JSON·스키마 SQL·PNG 내보내기.
---
...
파일 이름은 `.json`으로 끝나야 하며, `.erd`나 `.vuerd` 파일은 가져올 수 없습니다.
...
- JSON: 에디터에서 정의한 스키마 파일입니다. `<데이터베이스 이름>-<시간>.erd.json`으로 저장됩니다.
