...
테이블과 메모 명령을 먼저 다루고, 이어서 테이블 속성, 인덱스, 자동 배치, 데이터베이스 벤더를 다루며, 캔버스 자체는 캔버스 툴바, 확대/축소, 화면 이동, Diff Viewer 순으로 마지막에 다룹니다.
...
나머지 셋은 메인 스레드 밖에서 한 번에 계산되므로 지켜볼 것이 없습니다. 계산하는 동안 알림이 떠 있고, 완성된 배치는 다이어그램이 이미 차지하던 영역의 가운데에 놓입니다. `Cancel` 또는 `Escape`는 더 이상 기다리지 않는 배치를 버리며, shared worker를 실행하지 않는 호스트에서는 `Could not place tables`로 끝납니다. [Web Worker](../../api/installation.md#web-worker) 문서를 참고하세요.
